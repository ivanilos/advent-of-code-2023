...
fn solve(rows: Vec<Vec<char>>, records: Vec<Vec<usize>>) -> u64 {
    ...
    for i in 0..rows.len() {
        ...
        let mut unfolded_row: Vec<char> = rows[i].to_vec();
        let mut unfolded_record: Vec<usize> = records[i].to_vec();

        for _ in 0..4 {
            unfolded_row.push('?');
            unfolded_row.extend(&rows[i]);
            unfolded_record.extend(&records[i]);
        }

        result += get_ways(0, 0, &unfolded_row, &unfolded_record, &mut memo);
    }
