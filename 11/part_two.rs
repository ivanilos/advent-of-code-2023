...
const EXPANSION_SIZE: u64 = 1000000;
...
fn solve(grid: Vec<Vec<char>>) -> u64 {
    let mut x_qt: Vec<u64> = vec![0; grid.len()];
    let mut y_qt: Vec<u64> = vec![0; grid[0].len()];

    let mut x_empty: Vec<u64> = vec![0; grid.len()];
    let mut y_empty: Vec<u64> = vec![0; grid[0].len()];
    ...
    for i in 0..grid.len() {
        if x_qt[i] == 0 {
            x_empty[i] = EXPANSION_SIZE - 1;
        }
    }

    for j in 0..(grid[0].len()) {
        if y_qt[j] == 0 {
            y_empty[j] = EXPANSION_SIZE - 1;
        }
    }

    let mut result: u64 = 0;
    let mut cur_qt: u64 = 0;
    for i in 0..x_qt.len() {
