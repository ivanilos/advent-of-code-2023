...
const MIN_STEPS_ON_DIR: i32 = 4;
const MAX_STEPS_ON_DIR: i32 = 10;
...
fn dijkstra(x: i32, y: i32, input: Vec<Vec<u32>>) -> u32 {
    ...
    while let Some(Reverse((cost, node))) = heap.pop() {
        ...
        for i in -1..=1 {
            ...
            for step in 1..=MAX_STEPS_ON_DIR {
                let nx = x + step * DX[((dir + i + DIRS) % DIRS) as usize];
                let ny = y + step * DY[((dir + i + DIRS) % DIRS) as usize];
                let n_dir = (dir + i + DIRS) % DIRS;
                let n_steps_on_dir = if i == 0 { steps_on_dir + step } else { step };

                if is_in(nx, ny, &input) {
                    cur_cost += input[nx as usize][ny as usize];
                    let n_cost = cur_cost;

                    if n_steps_on_dir < MIN_STEPS_ON_DIR || n_steps_on_dir > MAX_STEPS_ON_DIR {
                        continue;
                    }
    ...
    let mut result = INF;
    for dir in 0..DIRS {
        for steps_on_dir in MIN_STEPS_ON_DIR..=MAX_STEPS_ON_DIR {
            let node = Node {
                x: target_x as i32,
                y: target_y as i32,
                dir: dir as i32,
